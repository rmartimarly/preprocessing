...
#import cv2
#path = '../AllDICOMs/'
path = 'C:/img/INbreast Release 1.0/'
xrange = range
...
def loadim(fname, preprocesspath=preprocesspath, aug = False ):
...
def loaddata(fold, totalfold, usedream=True, aug=True):
  ...
  #if usedream:
    #rm outx, outy = extractdreamdata()
    #rm traindata = np.concatenate((traindata,outx), axis=0)
    #rm trainlabel = np.concatenate((trainlabel,outy), axis=0)
